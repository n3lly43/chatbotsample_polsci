...
def test_build_prompt_kb_overview_with_braces():
    """KB overview containing {braces} must pass through as single braces."""
    from src.prompts import build_prompt
    ...
    assert "{{variable_name}}" not in prompt  # no double-bracing
    assert "{R}" in prompt
    assert "KNOWLEDGE BASE OVERVIEW" in prompt
...
def test_qu_prompt_kb_overview_with_braces():
    ...
    assert "{id}" in prompt
    assert "{{id}}" not in prompt  # no double-bracing
