...
**Date:** 2026-03-05
**Status:** Implemented
**Last updated:** 2026-03-06 (second multi-agent debug review: 3 more bugs fixed — dedup suffix collision, defensive dict access in format_sql_results, case-insensitive route parsing)
...
```json
{
  "pts_dataset__pts_data_csv": {
    ...
    "table_description": "Political Terror Scale dataset measuring state-sanctioned violence",
    "columns": [
      {
        "name": "Country", "type": "TEXT",
        "sample": ["Brazil", "China", "India"],
        "stats": {"unique_count": 187},
        "description": "Country name"
      },
      {
        "name": "Year", "type": "INTEGER",
        "sample": ["1976", "1998", "2020"],
        "stats": {"unique_count": 45, "min": 1976, "max": 2020},
        "description": "Year of observation"
      },
      {
        "name": "PTS_A", "type": "REAL",
        "sample": ["1.0", "3.0", "5.0"],
        "stats": {"unique_count": 9, "min": 1.0, "max": 5.0},
        "description": "Amnesty International PTS score (1-5 scale)"
      }
    ],
    "row_count": 15234
  }
...
- `sample`: Evenly-spaced representative values from the sorted unique set (not first-N). Numeric columns are sorted numerically; text columns alphabetically.
- `stats`: `unique_count` (all types), `min`/`max` (INTEGER and REAL only)
- `description`: Semantic meaning from codebook or LLM inference (see Schema Enrichment below)

Table-level `table_description` provides a one-sentence summary of the dataset.
...
### Schema enrichment (codebook detection + LLM descriptions)

After all tables are loaded, the ingestion pipeline enriches the schema with semantic column descriptions. This is a two-step process:

**Step 1 — Codebook detection:** For each tabular file, search its directory for documentation files (`.pdf`, `.docx`, `.txt`, `.md`). Files with keywords like "codebook", "readme", "dictionary", "documentation" are prioritized. If found, the codebook text is read (up to 4000 chars) using the existing readers registry.

**Step 2 — LLM description:** An LLM call generates:
- `table_description`: One-sentence summary of the dataset
- Per-column `description`: Semantic meaning of each column

The LLM receives column names, types, sample values, and statistics. If codebook text was found, it is included in the prompt for interpretation. If no codebook exists, the LLM infers from column metadata alone.

**Graceful degradation:** LLM failure during enrichment is non-fatal — the pipeline logs a warning and continues. The schema works identically without descriptions; they are optional enhancements.

**Key functions:**
- `_find_codebook_files(tabular_file)` — searches same directory for documentation files
- `_read_codebook_text(codebook_files)` — reads and concatenates codebook content
- `_describe_columns_with_llm(table_name, source_file, columns_info, codebook_text, cfg)` — calls LLM, returns `{table_description, columns: {name: desc}}`
- `_enrich_schema_with_descriptions(schema_registry, tabular_files, documents_dir, cfg)` — orchestrates the enrichment for all tables
...
- CSV files are read with `utf-8-sig` encoding to handle Excel-exported BOM
...
When `sql_db/sql_schemas.json` exists, a compact summary is appended to the QU prompt. The summary includes column types, statistics, sample values, and descriptions:

```
Available SQL tables:

- pts_dataset__pts_data_csv (15234 rows, from PTS_dataset/pts_data.csv):
  Description: Political Terror Scale dataset measuring state-sanctioned violence
    Country (TEXT, 187 unique) e.g. "Brazil", "China", "India" — Country name
    Year (INTEGER, range 1976–2020, 45 unique) e.g. "1976", "1998", "2020" — Year of observation
    PTS_A (REAL, range 1.0–5.0, 9 unique) e.g. "1.0", "3.0", "5.0" — Amnesty International PTS score (1-5 scale)
```

This gives the LLM the context it needs to generate correct WHERE clauses (exact string values), appropriate aggregations (knowing value ranges), and accurate column references. Schema text goes through `_escape_braces()` before prompt injection.
...
### `format_sql_results_as_context(rows, sql_query, source_file, table_info=None) -> str`
...
Dataset description: Political Terror Scale dataset measuring state-sanctioned violence

Column descriptions:
  Country: Country name
  Year: Year of observation
  PTS_A: Amnesty International PTS score (1-5 scale)
...
- When `table_info` is provided (from schema registry), includes dataset description and column descriptions so the response LLM can correctly interpret the data
...
|---|---|---|---|
| `paths.sql_db` | `src/sql_ingest.py`, `src/sql_retriever.py`, `src/query_engine.py`, `src/retriever.py` | `"sql_db"` | Directory for SQLite DB + schema registry |
| `sql.enabled` | `src/retriever.py`, `src/query_engine.py`, `src/ingest.py` | `true` | Can disable if no tabular data |
...
| LLM unavailable during schema enrichment | Non-fatal warning, schema saved without descriptions, system works identically |
| No codebook found near tabular file | LLM infers column descriptions from names/types/samples alone |
| Codebook found but unreadable | Skipped, falls back to inference-only description |
