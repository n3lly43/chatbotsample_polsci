...
**Key principle:** The reformulated query is used ONLY for retrieval (embedding search). The original user query is still passed to `verify_and_respond()` for response generation, so the LLM answers what the user actually asked.
    ...
    Returns:
        {
            ...
            "search_query": "reformulated query for embedding search",
...
# Use reformulated query for retrieval, original for response
search_query = qu_result["search_query"]
original_query = qu_result["original_query"]
...
- The original query is preserved for response generation — the LLM answers what the user actually asked, not a reformulated version
...
1. `test_understand_query_reformulates` — Clear query returns action="search" with reformulated query
2. `test_understand_query_clarifies` — Ambiguous query returns action="clarify"
3. `test_understand_query_preserves_original` — Original query always in result
4. `test_understand_query_with_history` — Pronoun resolution using conversation history
5. `test_understand_query_disabled` — When disabled, returns raw query passthrough
6. `test_understand_query_fallback_on_error` — LLM failure falls back to raw query
